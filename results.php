...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .nav-icons a { background: rgba(255,255,255,0.3); width: 44px; height: 44px; display: flex; align-items: center; justify-content: center; border-radius: 10px; color: var(--text-color); text-decoration: none; }
    ...
    #theme-selector button:hover {
      ...
      box-shadow: 0 3px 8px rgba(0,0,0,0.15);
        ...
        .question-container { background: var(--card-bg); backdrop-filter: blur(12px); border-radius: 20px; padding: 20px; border: 1px solid var(--card-border); max-width: 700px; width: 100%; }
        ...
        .score-circle { width: 80px; height: 80px; border-radius: 50%; background: var(--btn-grad-start); color: var(--btn-text); font-size: 24px; display: flex; align-items: center; justify-content: center; margin-bottom: 10px; }
        ...
        .type-card { background: var(--card-bg); border: 1px solid var(--card-border); border-radius: 18px; padding: 20px; flex: 1; min-width: 250px; display: flex; flex-direction: column; align-items: center; text-align: center; }
        .type-icon { width: 60px; height: 60px; border-radius: 50%; background: var(--btn-grad-start); display: flex; align-items: center; justify-content: center; font-size: 28px; color: var(--btn-text); margin-bottom: 15px; }
        ...
        .btn { padding: 15px 30px; background: linear-gradient(135deg, var(--btn-grad-start), var(--btn-grad-end)); border: none; border-radius: 50px; color: var(--btn-text); font-weight: bold; cursor: pointer; text-decoration: none; display: flex; align-items: center; gap: 10px; }
        @media (max-width:768px) { .scores { flex-direction: column; gap: 20px; } .type-container { flex-direction: column; } }
    </style>
</head>
<body>
    <div class="container">
        <header>
            ...
            <div>
                <a href="dashboard.php" style="color: var(--text-color); text-decoration: none;"><i class="fas fa-user"></i> <?= htmlspecialchars($_SESSION['username']) ?></a>
            </div>
        </header>
        <div class="content">
                    ...
                    <?php foreach ($dominantTypes as $type): ?>
                        <div class="type-card">
                            ...
                            <div class="traits-container">
                                <?php foreach ($typeInfo[$type]['strengths'] as $s): ?><div class="trait strength"><?= htmlspecialchars($s) ?></div><?php endforeach; ?>
                            </div>
                            <div class="traits-container">
                                <?php foreach ($typeInfo[$type]['weaknesses'] as $w): ?><div class="trait weakness"><?= htmlspecialchars($w) ?></div><?php endforeach; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
